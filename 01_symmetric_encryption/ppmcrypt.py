...
def task1():
    ...
    with open('gcm_encrypted.ppm', 'wb') as f:
            image.write_to_file(f)
    image.decrypt(key)
    with open('gcm_decrypted.ppm', 'wb') as f:
            image.write_to_file(f)
    return 
...
def task2():
    ...
    with open('dk.ppm', 'rb') as f:
        dk = PPMImage.load_from_file(f)
    with open('se.ppm', 'rb') as f:
        se = PPMImage.load_from_file(f)

    for i in range(len(se.data)):
        image.data[i] ^= dk.data[i] ^ se.data[i]

    with open('ctr_encrypted_tampered.ppm', 'wb') as f:
            image.write_to_file(f)
    image.decrypt(key)
    with open('ctr_decrypted.ppm', 'wb') as f:
            image.write_to_file(f)
    return 
...
def task3():
    ...
    with open('gcm_encrypted_tampered.ppm', 'wb') as f:
            image.write_to_file(f)
    image.decrypt(key)
    with open('gcm_decrypted.ppm', 'wb') as f:
            image.write_to_file(f)
